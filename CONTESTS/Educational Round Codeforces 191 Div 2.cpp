...
void solve() {
    ...
    long long noAiSpeed = x + y;
    long long noAiHour = (n + noAiSpeed - 1) / noAiSpeed;

    long long aiHours = 0;
    long long maximTime = (n + x - 1) / x;

    if (maximTime <= z) {
        aiHours = maximTime;
    }
    else {
        long long linesDuringSetup = x * z;
        long long linesLeft = n - linesDuringSetup;

        long long aiSpeed = x + (10 * y);

        long long hoursLeft =
            (linesLeft + aiSpeed - 1) / aiSpeed;

        aiHours = z + hoursLeft;
    }

    cout << min(noAiHour, aiHours) << "\n";
}
